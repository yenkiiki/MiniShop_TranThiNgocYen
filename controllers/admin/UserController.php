...
class UserController {
    ...
    public function store(): void {
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if (empty($fullName) || empty($userName) || empty($password) || empty($email)) {
                header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user/create?error=missing_fields");
                exit();
            }
            ...
            try {
                ...
                if ($this->userDAO->insert($user)) {
                    header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?msg=insert_success");
                    exit();
                }
            } catch (Exception $e) {
                header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user/create?error=exception");
                exit();
            }
        }
    }
    ...
    public function edit(): void {
        ...
        if (!$user) {
            header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?msg=not_found");
            exit();
        }
    ...
    public function update(): void {
        ...
        if (!$user || $_SERVER["REQUEST_METHOD"] !== "POST") {
            header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user");
            exit();
        }
        ...
        if (empty($fullName) || empty($userName) || empty($email)) {
            header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user/edit/$id?error=missing_fields");
            exit();
        }
        ...
        try {
            ...
            if ($this->userDAO->update($user)) {
                header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?msg=update_success");
                exit();
            }
        } catch (Exception $e) {
            header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user/edit/$id?error=exception");
            exit();
        }
    }
        ...
        if ($id > 0) {
            try {
                if ($this->userDAO->delete($id)) {
                    header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?msg=delete_success");
                    exit();
                } else {
                    header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?error=delete_fail");
                    exit();
                }
            } catch (Exception $e) {
                header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user?error=delete_exception");
                exit();
            }
        }
        header("Location: /MINISHOP_TRANTHINGOCYEN/admin/user");
